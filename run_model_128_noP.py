...
if __name__ =="__main__":
    ...
    parser.add_argument('--phy_scale', type = float, default= 0.5)
...
########### loaddata ############
    ...
    trianset,testValset = random_split(dataset,[0.8,0.2],generator=torch.Generator().manual_seed(0))
    valset,testset = random_split(dataset,[0.5,0.5],generator=torch.Generator().manual_seed(0))
    ...
    for epoch in range(nb_epochs):
        ...
        print(f"Epoch {epoch}. Training loss: {epoch_loss / len(trainloader)}, Data loss: {data_loss.item()}, Phy Loss: {phy_loss}")
